...
## SOL-018 — Multi-host model dir přes env var (Starfield migration)

- **Problém:** Při migraci Eleutheria runtime z `kqs-arch` (laptop, user
  `lvx`) na **Starfield** (Ubuntu 24.04 VM, user `sofie`, RTX PRO 4000 +
  CUDA 13.0) první smoke spuštění `eleutheria --cuda -p "..." -n 200`
  selhalo s `Error: No such file or directory (os error 2)`. Výpis
  ukázal `Model: 1.5b (/home/lvx/Models/falcon-h1-1.5b-instruct)` —
  shorthand `1.5b` byl hardcoded na laptop path, který na Starfieldu
  neexistuje (`/home/sofie/Models/...`).
- **Kořenová příčina:** `main.rs:444-445` měl shorthand resolution jako:
  ```rust
  "1.5b" => PathBuf::from("/home/lvx/Models/falcon-h1-1.5b-instruct"),
  "7b" => PathBuf::from("/home/lvx/Models/falcon-h1-7b-instruct"),
  ```
  Plus 3 testovací moduly (`falcon_h1::config`, `training::dataset`,
  `training::checkpoint`) měly stejnou hardcoded cestu pro dev model
  dependency. Z těch tří jen `dataset` a `checkpoint` měly elegant skip
  pattern (`if !path.exists() { return None; }`); `config::test_load_config`
  použil `.unwrap()` → panic na hostovi bez modelu.
- **Řešení:** Nový helper `default_models_dir()` v `main.rs`:
  ```rust
  ...
  ```
  Konzistentní s `default_session_path()` / `default_core_memory_path()`
  patterny. Shorthand resolution pak `default_models_dir().join(...)`.
  Tři testovací moduly stejný pattern (env var override + skip if not
  exists) napříč všemi třemi konzistentně.
- **Ponaučení:**
  1. **Hardcoded user paths jsou single-host time bomby.** Funguje na
     dev stroji, padá při prvním deploymentu. Když píšeš `/home/lvx/...`,
     ptej se "co když tohle běží jinde?". Pokud odpověď "vždy poběží jen
     tady", tak ok — jinak `$HOME` nebo env var.
  2. **Konzistentní fallback pattern napříč moduly.** Když 4 místa
     potřebují stejnou cestu, fakticky-jeden helper s konzistentním
     skip patternem v testech a env override pro non-default hosty.
     Mix patternů (jeden test panic, druhý skip) maskuje chybu —
     unify chování dělá failure modes předvídatelnější.
  3. **Runtime fallback `$HOME/Models`, test fallback `/home/lvx/Models`.**
     Záměrná asymetrie: runtime dělá best-effort multi-host (HOME-relative),
     testy zachovávají dev fallback pro skip pattern (`path.exists()`
     vrátí false na CI/jiném hostu, test elegantně skipne).

## SOL-019 — Hairpin NAT bypass přes /etc/hosts pro headscale klienty

- **Problém:** Při registraci nového Tailscale klienta (Starfield,
  192.168.1.50) do self-hosted headscale (`hekate.lomsky.net`)
  `sudo tailscale up --login-server=https://hekate.lomsky.net ...`
  visel s timeoutem. `tailscaled` log:
  ```
  Received error: fetch control key: Get "https://hekate.lomsky.net/key?v=133":
  dial tcp 93.153.46.192:443: connect: connection timed out
  ```
  `93.153.46.192` je T-Mobile WAN IP routeru. Z laptopu (jiný stroj na
  stejné LAN) tailscale klient fungoval — měl `ESTAB` connection na
  93.153.46.192:443. Tedy hairpin NAT pro hekate fungoval pro laptop,
  ne pro Starfield, na téže L2 síti.
- **Kořenová příčina:** `hekate.lomsky.net` je veřejné DNS jméno
  pointující na WAN IP domácího routeru (`93.153.46.192`); router má
  port forward 443 → hefaistos LXC (`192.168.1.20`) kde běží headscale
  Docker container za Caddy reverse proxy. **Hairpin NAT** (LAN klient
  → WAN IP same router → port forward zpět dovnitř) router podporoval
  pro některé klienty, ne pro jiné. Důvod asymetrie nebyl
  diagnostikován do hloubky (TP-Link router-specific firmware
  chování); pravděpodobně NAT session affinity nebo per-source-port
  rate-limit.
- **Řešení:** `/etc/hosts` override na Starfieldu:
  ```bash
  echo "192.168.1.20 hekate.lomsky.net" | sudo tee -a /etc/hosts
  ```
  Starfield pak komunikuje přímo na hefaistos LAN IP, bez hairpinu
  přes router. TLS cert validní (Caddy drží valid Let's Encrypt cert
  pro `hekate.lomsky.net` na hefaistos). `tailscale up` exit 0 v <5s,
  Starfield zaregistrován jako `100.64.0.8 starfield rodina linux`.
- **Ponaučení:**
  1. **Hairpin NAT není reliable pro homelab cluster.** Když máš public
     DNS pointující na WAN IP a klienti za NATem, nepředpokládej, že
     hairpin funguje pro každého klienta. Test s konkrétním klientem,
     ne extrapoluj.
  2. **`/etc/hosts` override > router config tweak.** Když cílem je
     "client X dostane hostname Y", local hosts file je deterministicky
     správný flow, eliminuje router proměnlivost. Router config tweak
     by ovlivnil všechny klienty (může rozbít fungující path pro
     laptop). Local override = surgical.
  3. **Diagnose by comparison.** "Funguje na X, nefunguje na Y na téže
     LAN" je nejostřejší symptom — vyloučí firewall, vyloučí DNS
     (oba dostávají stejný A record), nechává explicitně middleware
     (router NAT chování). Bez té komparace bych se možná zaprdla
     debugem na hefaistos straně.
  4. **Tailscale `--login-server` HTTPS endpoint je critical path.**
     Bez něj žádný klient se nezaregistruje. Pokud hairpin selhává,
     `/etc/hosts` override musí být první workaround před hlubším
     debugem (zbytečně se nehrabat v routeru, když 5 sekund hosts
     entry to fixne).
